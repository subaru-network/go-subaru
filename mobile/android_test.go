...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gsu.LightScryptN, Gsu.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.subaru", new NodeConfig());
			node.start();
			...
			SubaruClient ec = node.getSubaruClient();
	...
	// signatures too. Regression test for go-subaru issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/subaru/gsutest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gsu and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.subaru", "github.com/subaru-network/go-subaru/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "gsu.aar"), "gsu.aar")

	if err = os.WriteFile(filepath.Join("src", "androidTest", "java", "org", "subaru", "gsutest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
