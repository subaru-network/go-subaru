...
import (
	...
	"github.com/subaru-network/go-subaru/accounts/usbwallet/trezor"
...
// or whether the Subaru app was not started on it.
...
// Subaru address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.SubaruAddress)
	if _, err := w.trezorExchange(&trezor.SubaruGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.SubaruTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.SubaruTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
