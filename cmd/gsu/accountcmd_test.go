...
`
	...
	{
		gsu := runGsu(t, "account", "list", "--datadir", datadir)
		gsu.Expect(want)
		gsu.ExpectExit()
	}
	{
		gsu := runGsu(t, "--datadir", datadir, "account", "list")
		gsu.Expect(want)
		gsu.ExpectExit()
	}
}
...
func TestAccountNew(t *testing.T) {
	gsu := runGsu(t, "account", "new", "--lightkdf")
	defer gsu.ExpectExit()
	gsu.Expect(`
...
func TestAccountHelp(t *testing.T) {
	gsu := runGsu(t, "account", "-h")
	gsu.WaitExit()
	if have, want := gsu.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}

	gsu = runGsu(t, "account", "import", "-h")
	gsu.WaitExit()
	if have, want := gsu.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	gsu := runGsu(t, "--lightkdf", "account", "import", "-password", passwordFile, keyfile)
	defer gsu.ExpectExit()
	gsu.Expect(expected)
}
...
func TestWalletImport(t *testing.T) {
	gsu := runGsu(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gsu.ExpectExit()
	gsu.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := os.ReadDir(filepath.Join(gsu.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	gsu := runMinimalGsu(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gsu.StderrText(), m) {
...
// https://github.com/subaru-network/go-subaru/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gsu.SetTemplateFunc("keypath", func(file string) string {
