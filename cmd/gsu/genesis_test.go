...
// Tests that initializing Gsu with a custom genesis block and chain definitions
// work properly.
func TestCustomGenesis(t *testing.T) {
	for i, tt := range customGenesisTests {
		...
		runGsu(t, "--datadir", datadir, "init", json).WaitExit()

		// Query the custom genesis block
		gsu := runGsu(t, "--networkid", "1337", "--syncmode=full", "--cache", "16",
			"--datadir", datadir, "--maxpeers", "0", "--port", "0", "--authrpc.port", "0",
			"--nodiscover", "--nat", "none", "--ipcdisable",
			"--exec", tt.query, "console")
		gsu.ExpectRegexp(tt.result)
		gsu.ExpectExit()
	}
}
