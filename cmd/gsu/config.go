...
import (
	...
	"github.com/subaru-network/go-subaru/accounts/external"
	"github.com/subaru-network/go-subaru/accounts/keystore"
	"github.com/subaru-network/go-subaru/accounts/scwallet"
	"github.com/subaru-network/go-subaru/accounts/usbwallet"
...
func loadConfig(file string, cfg *gsuConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
// makeConfigNode loads gsu configuration and creates a blank node instance.
func makeConfigNode(ctx *cli.Context) (*node.Node, gsuConfig) {
	// Load defaults.
	cfg := gsuConfig{
...
// makeFullNode loads gsu configuration and creates the Subaru backend.
func makeFullNode(ctx *cli.Context) (*node.Node, ethapi.Backend) {
	...
	if eth != nil && !ctx.IsSet(utils.IgnoreLegacyReceiptsFlag.Name) {
		...
		if err != nil {
			...
			stack.Close()
			utils.Fatalf("Database has receipts with a legacy format. Please run `gsu db freezer-migrate`.")
		}
	}
	...
	// Add the Subaru Stats daemon if requested.
	if cfg.Ethstats.URL != "" {
...
func applyMetricConfig(ctx *cli.Context, cfg *gsuConfig) {
