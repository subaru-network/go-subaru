...
// Package utils contains internal helper functions for go-subaru commands.
package utils
...
import (
	...
	"github.com/subaru-network/go-subaru/accounts/keystore"
	"github.com/subaru-network/go-subaru/common"
	"github.com/subaru-network/go-subaru/common/fdlimit"
	"github.com/subaru-network/go-subaru/consensus"
	"github.com/subaru-network/go-subaru/consensus/ethash"
	...
	"github.com/subaru-network/go-subaru/crypto"
	"github.com/subaru-network/go-subaru/eth"
	ethcatalyst "github.com/subaru-network/go-subaru/eth/catalyst"
	"github.com/subaru-network/go-subaru/eth/downloader"
	"github.com/subaru-network/go-subaru/eth/ethconfig"
	"github.com/subaru-network/go-subaru/eth/filters"
	"github.com/subaru-network/go-subaru/eth/gasprice"
	"github.com/subaru-network/go-subaru/eth/tracers"
	"github.com/subaru-network/go-subaru/ethdb"
	"github.com/subaru-network/go-subaru/ethdb/remotedb"
	"github.com/subaru-network/go-subaru/ethstats"
	"github.com/subaru-network/go-subaru/graphql"
	...
	lescatalyst "github.com/subaru-network/go-subaru/les/catalyst"
	"github.com/subaru-network/go-subaru/log"
	"github.com/subaru-network/go-subaru/metrics"
	"github.com/subaru-network/go-subaru/metrics/exp"
	"github.com/subaru-network/go-subaru/metrics/influxdb"
	...
	"github.com/subaru-network/go-subaru/p2p/netutil"
...
var (
	...
	MinerSubaseFlag = &cli.StringFlag{
	...
	RPCGlobalTxFeeCapFlag = &cli.Float64Flag{
		Name:     "rpc.txfeecap",
		Usage:    "Sets a cap on transaction fee (in su) that can be sent via the RPC APIs (0 = no cap)",
	...
	IgnoreLegacyReceiptsFlag = &cli.BoolFlag{
		...
		Usage:    "Gsu will start up even if there are legacy receipts in freezer",
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.String(DataDirFlag.Name); path != "" {
		if ctx.Bool(RopstenFlag.Name) {
			// Maintain compatibility with older Gsu configurations storing the
...
// for Gsu and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `gsu account list`)")
	log.Warn("-------------------------------------------------------------------")
...
// setSubase retrieves the etherbase either from the directly specified
// command line flags or from the keystore if CLI indexed.
func setSubase(ctx *cli.Context, ks *keystore.KeyStore, cfg *ethconfig.Config) {
	// Extract the current etherbase
	var etherbase string
	if ctx.IsSet(MinerSubaseFlag.Name) {
		etherbase = ctx.String(MinerSubaseFlag.Name)
	}
	...
	if etherbase != "" {
		if ks != nil {
			...
			cfg.Miner.Subase = account.Address
		} else {
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	case ctx.Bool(DeveloperFlag.Name):
		...
		// setSubase has been called above, configuring the miner address from command line flags.
		if cfg.Miner.Subase != (common.Address{}) {
			developer = accounts.Account{Address: cfg.Miner.Subase}
...
// RegisterEthService adds an Subaru client to the stack.
// The second return value is the full node instance, which may be nil if the
// node is running as a light client.
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config) (ethapi.Backend, *eth.Subaru) {
	if cfg.SyncMode == downloader.LightSync {
		backend, err := les.New(stack, cfg)
		if err != nil {
			Fatalf("Failed to register the Subaru service: %v", err)
		}
...
// RegisterEthStatsService configures the Subaru Stats daemon and adds it to the node.
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := ethstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the Subaru Stats service: %v", err)
	}
}
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "gsu.", tagsMap)
		} else if enableExportV2 {
			tagsMap := SplitTagsFlag(ctx.String(MetricsInfluxDBTagsFlag.Name))

			log.Info("Enabling metrics export to InfluxDB (v2)")

			go influxdb.InfluxDBV2WithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, token, bucket, organization, "gsu.", tagsMap)
		}
