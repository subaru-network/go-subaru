...
// importGenesis imports a Gsu genesis spec into puppeth.
...
func (w *wizard) manageGenesis() {
	...
	case "2":
		...
		// Export the native genesis spec used by puppeth and Gsu
		gsuJson := filepath.Join(folder, fmt.Sprintf("%s.json", w.network))
		if err := os.WriteFile(gsuJson, out, 0644); err != nil {
			log.Error("Failed to save genesis file", "err", err)
			return
		}
		log.Info("Saved native genesis chain spec", "path", gsuJson)
