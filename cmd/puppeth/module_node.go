...
// nodeDockerfile is the Dockerfile required to run an Subaru node.
var nodeDockerfile = `
FROM subaru/client-go:latest
...
{{end}}
RUN \
  echo 'gsu --cache 512 init /genesis.json' > gsu.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.subaru/keystore/ && cp /signer.json /root/.subaru/keystore/' >> gsu.sh && \{{end}}
	echo $'exec gsu --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Subase}}--miner.etherbase {{.Subase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> gsu.sh

ENTRYPOINT ["/bin/sh", "gsu.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an Subaru node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.subaru{{if .Ethashdir}}
...
// deployNode deploys a new Subaru node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 gsu --exec admin.nodeInfo.enode --cache=16 attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.subaru"],
