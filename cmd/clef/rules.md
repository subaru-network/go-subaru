...
* I want to auto-approve transactions with contract `CasinoDapp`, with up to `0.05 su` in value to maximum `1 su` per 24h period
...
implemented for `gsu`. There are no known security vulnerabilities in, nor have we had any security-problems with it so far.
