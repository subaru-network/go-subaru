...
import (
	...
	"github.com/subaru-network/go-subaru/internal/utesting"
...
func TestEthSuite(t *testing.T) {
	gsu, err := runGsu()
	if err != nil {
		t.Fatalf("could not run gsu: %v", err)
	}
	defer gsu.Close()

	suite, err := NewSuite(gsu.Server().Self(), fullchainFile, genesisFile)
	if err != nil {
...
// runGsu creates and starts a gsu node
func runGsu() (*node.Node, error) {
...
func setupGsu(stack *node.Node) error {
