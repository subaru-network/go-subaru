// Copyright 2019 The go-subaru Authors
...
// Package les implements the Light Subaru Subprotocol.
package les
...
import (
	...
	"github.com/subaru-network/go-subaru/common/mclock"
	...
	"github.com/subaru-network/go-subaru/les/downloader"
	"github.com/subaru-network/go-subaru/les/vflux"
	vfc "github.com/subaru-network/go-subaru/les/vflux/client"
	"github.com/subaru-network/go-subaru/light"
...
func New(stack *node.Node, config *ethconfig.Config) (*LightSubaru, error) {
...
func (s *LightSubaru) VfluxRequest(n *enode.Node, reqs vflux.Requests) vflux.Replies {
	if !s.udpEnabled {
		return nil
	}
...
func (s *LightSubaru) vfxVersion(n *enode.Node) uint {
	if n.Seq() == 0 {
		var err error
		if !s.udpEnabled {
			return 0
...
func (s *LightSubaru) prenegQuery(n *enode.Node) int {
	if s.vfxVersion(n) < 1 {
...
// Subase is the address that mining rewards will be send to
func (s *LightDummyAPI) Subase() (common.Address, error) {
	return common.Address{}, fmt.Errorf("mining is not supported in light mode")
}

// Coinbase is the address that mining rewards will be send to (alias for Subase)
...
// APIs returns the collection of RPC services the subaru package offers.
// NOTE, some of these services probably need to be moved to somewhere else.
func (s *LightSubaru) APIs() []rpc.API {
...
func (s *LightSubaru) ResetWithGenesisBlock(gb *types.Block) {
	s.blockchain.ResetWithGenesisBlock(gb)
}

func (s *LightSubaru) BlockChain() *light.LightChain      { return s.blockchain }
func (s *LightSubaru) TxPool() *light.TxPool              { return s.txPool }
func (s *LightSubaru) Engine() consensus.Engine           { return s.engine }
func (s *LightSubaru) LesVersion() int                    { return int(ClientProtocolVersions[0]) }
func (s *LightSubaru) Downloader() *downloader.Downloader { return s.handler.downloader }
func (s *LightSubaru) EventMux() *event.TypeMux           { return s.eventMux }
func (s *LightSubaru) Merger() *consensus.Merger          { return s.merger }

// Protocols returns all the currently configured network protocols to start.
func (s *LightSubaru) Protocols() []p2p.Protocol {
...
// light subaru protocol implementation.
func (s *LightSubaru) Start() error {
