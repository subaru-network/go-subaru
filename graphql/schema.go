...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte Subaru address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an Subaru account at a particular block.
    type Account {
    ...
    # Transaction is an Subaru transaction.
    type Transaction {
    ...
    type Block {
        ...
        # Account fetches an Subaru account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an Subaru account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an Subaru block by number or by hash. If neither is
