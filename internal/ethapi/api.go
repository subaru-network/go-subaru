...
import (
	...
	"github.com/subaru-network/go-subaru/accounts/abi"
	"github.com/subaru-network/go-subaru/accounts/keystore"
	"github.com/subaru-network/go-subaru/accounts/scwallet"
	"github.com/subaru-network/go-subaru/common"
	"github.com/subaru-network/go-subaru/common/hexutil"
	"github.com/subaru-network/go-subaru/common/math"
	"github.com/subaru-network/go-subaru/consensus/ethash"
	"github.com/subaru-network/go-subaru/consensus/misc"
	"github.com/subaru-network/go-subaru/core"
	"github.com/subaru-network/go-subaru/core/state"
	...
	"github.com/subaru-network/go-subaru/eth/tracers/logger"
...
// SubaruAPI provides an API to access Subaru related information.
type SubaruAPI struct {
	b Backend
}

// NewSubaruAPI creates a new Subaru protocol API.
func NewSubaruAPI(b Backend) *SubaruAPI {
	return &SubaruAPI{b}
}

// GasPrice returns a suggestion for a gas price for legacy transactions.
func (s *SubaruAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
...
func (s *SubaruAPI) MaxPriorityFeePerGas(ctx context.Context) (*hexutil.Big, error) {
	tipcap, err := s.b.SuggestGasTipCap(ctx)
	if err != nil {
		return nil, err
	}
...
func (s *SubaruAPI) FeeHistory(ctx context.Context, blockCount rpc.DecimalOrHex, lastBlock rpc.BlockNumber, rewardPercentiles []float64) (*feeHistoryResult, error) {
...
func (s *SubaruAPI) Syncing() (interface{}, error) {
...
// SubaruAccountAPI provides an API to access accounts managed by this node.
// It offers only methods that can retrieve accounts.
type SubaruAccountAPI struct {
	am *accounts.Manager
}

// NewSubaruAccountAPI creates a new SubaruAccountAPI.
func NewSubaruAccountAPI(am *accounts.Manager) *SubaruAccountAPI {
	return &SubaruAccountAPI{am: am}
}

// Accounts returns the collection of accounts this node manages.
func (s *SubaruAccountAPI) Accounts() []common.Address {
...
// Sign calculates an Subaru ECDSA signature for:
// keccak256("\x19Subaru Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/subaru-network/go-subaru/wiki/Management-APIs#personal_sign
...
// the address of:
// hash = keccak256("\x19Subaru Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be 27 or 28 for legacy reasons.
//
// https://github.com/subaru-network/go-subaru/wiki/Management-APIs#personal_ecRecover
func (s *PersonalAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid Subaru signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and gsu.
...
// BlockChainAPI provides an API to access Subaru blockchain data.
type BlockChainAPI struct {
	b Backend
}

// NewBlockChainAPI creates a new Subaru blockchain API.
func NewBlockChainAPI(b Backend) *BlockChainAPI {
	return &BlockChainAPI{b}
}

// ChainId is the EIP-155 replay-protection chain id for the current Subaru chain config.
//
...
// See: https://github.com/subaru-network/wiki/wiki/JSON-RPC-Error-Codes-Improvement-Proposal
...
//
// https://github.com/subaru-network/wiki/wiki/JSON-RPC#eth_sign
...
// DebugAPI is the collection of Subaru APIs exposed over the debugging
// namespace.
...
// Version returns the current subaru protocol version.
...
func checkTxFee(gasPrice *big.Int, gas uint64, cap float64) error {
	...
	feeEth := new(big.Float).Quo(new(big.Float).SetInt(new(big.Int).Mul(gasPrice, new(big.Int).SetUint64(gas))), new(big.Float).SetInt(big.NewInt(params.Su)))
	feeFloat, _ := feeEth.Float64()
	if feeFloat > cap {
		return fmt.Errorf("tx fee (%.2f su) exceeds the configured cap (%.2f su)", feeFloat, cap)
	}
	return nil
}
