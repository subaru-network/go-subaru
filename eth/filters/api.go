...
// information related to the Subaru protocol such als blocks, transactions and logs.
...
func (api *FilterAPI) Logs(ctx context.Context, crit FilterCriteria) (*rpc.Subscription, error) {
	...
	logsSub, err := api.events.SubscribeLogs(subaru.FilterQuery(crit), matchedLogs)
	if err != nil {
		return nil, err
	}
...
// Same as subaru.FilterQuery but with UnmarshalJSON() method.
type FilterCriteria subaru.FilterQuery
...
func (api *FilterAPI) NewFilter(crit FilterCriteria) (rpc.ID, error) {
	...
	logsSub, err := api.events.SubscribeLogs(subaru.FilterQuery(crit), logs)
	if err != nil {
		return "", err
	}
