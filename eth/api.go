...
import (
	...
	"github.com/subaru-network/go-subaru/trie"
)

// SubaruAPI provides an API to access Subaru full node-related information.
type SubaruAPI struct {
	e *Subaru
}

// NewSubaruAPI creates a new Subaru protocol API for full nodes.
func NewSubaruAPI(e *Subaru) *SubaruAPI {
	return &SubaruAPI{e}
}

// Subase is the address that mining rewards will be send to.
func (api *SubaruAPI) Subase() (common.Address, error) {
	return api.e.Subase()
}

// Coinbase is the address that mining rewards will be send to (alias for Subase).
func (api *SubaruAPI) Coinbase() (common.Address, error) {
	return api.Subase()
}

// Hashrate returns the POW hashrate.
func (api *SubaruAPI) Hashrate() hexutil.Uint64 {
	return hexutil.Uint64(api.e.Miner().Hashrate())
}

// Mining returns an indication if this node is currently mining.
func (api *SubaruAPI) Mining() bool {
...
func NewMinerAPI(e *Subaru) *MinerAPI {
	return &MinerAPI{e}
}
...
// SetSubase sets the etherbase of the miner.
func (api *MinerAPI) SetSubase(etherbase common.Address) bool {
	api.e.SetSubase(etherbase)
	return true
}
...
// AdminAPI is the collection of Subaru full node related APIs for node
// administration.
...
func NewAdminAPI(eth *Subaru) *AdminAPI {
...
// DebugAPI is the collection of Subaru full node APIs for debugging the
// protocol.
...
func NewDebugAPI(eth *Subaru) *DebugAPI {
