...
// the process. This is usually the case when Gsu is shutting down and some events
...
//
// Opposed to the original Subaru block synchronization which is trustless (and
...
// not from within (vs. legacy Subaru sync).
//
...
func (s *skeleton) startup() {
	...
	for {
		...
		case errc := <-s.terminate:
			// No head was announced but Gsu is shutting down
			errc <- nil
			return
		...
		case event := <-s.headEvents:
			...
			for {
				...
				default:
					...
					// error. Abort and wait until Gsu requests a termination.
					errc := <-s.terminate
					errc <- err
					return
				}
