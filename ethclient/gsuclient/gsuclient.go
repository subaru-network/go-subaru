...
// Package gsuclient provides an RPC client for gsu-specific APIs.
package gsuclient
...
// Client is a wrapper around rpc.Client that implements gsu-specific functionality.
//
// If you want to use the standardized Subaru RPC functionality, use ethclient.Client instead.
type Client struct {
	c *rpc.Client
}
...
func (ec *Client) CreateAccessList(ctx context.Context, msg subaru.CallMsg) (*types.AccessList, uint64, string, error) {
...
func (ec *Client) CallContract(ctx context.Context, msg subaru.CallMsg, blockNumber *big.Int, overrides *map[common.Address]OverrideAccount) ([]byte, error) {
...
// GCStats retrieves the current garbage collection stats from a gsu node.
...
// MemStats retrieves the current memory stats from a gsu node.
...
// GetNodeInfo retrieves the node info of a gsu node.
