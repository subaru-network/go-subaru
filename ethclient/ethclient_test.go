...
// Verify that Client implements the subaru interfaces.
var (
	_ = subaru.ChainReader(&Client{})
	_ = subaru.TransactionReader(&Client{})
	_ = subaru.ChainStateReader(&Client{})
	_ = subaru.ChainSyncReader(&Client{})
	_ = subaru.ContractCaller(&Client{})
	_ = subaru.GasEstimator(&Client{})
	_ = subaru.GasPricer(&Client{})
	_ = subaru.LogFilterer(&Client{})
	_ = subaru.PendingStateReader(&Client{})
	// _ = subaru.PendingStateEventer(&Client{})
	_ = subaru.PendingContractCaller(&Client{})
)
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	// Create Subaru Service
	config := &ethconfig.Config{Genesis: genesis}
	config.Ethash.PowMode = ethash.ModeFake
	ethservice, err := eth.New(n, config)
	if err != nil {
		t.Fatalf("can't create new subaru service: %v", err)
	}
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == subaru.NotFound {
		t.Fatal("error should not be nil/notfound")
	}

	// Test tx in block not found.
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 20); err != subaru.NotFound {
		t.Fatal("error should be subaru.NotFound")
	}
}
