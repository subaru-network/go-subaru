...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gsu is "gsu". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gsu", keep that.
	if name == "gsu" || name == "gsu-testnet" {
		name = "Gsu"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gsu" instances.
var isOldGsuResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gsu 1.4 are used if they exist.
	if warn, isOld := isOldGsuResource[path]; isOld {
		oldpath := ""
		if c.name() == "gsu" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGsuResourceWarning, "Using deprecated resource file %s, please move this file to the 'gsu' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func getKeyStoreDir(conf *Config) (string, bool, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = os.MkdirTemp("", "go-subaru-keystore")
		isEphemeral = true
	}
