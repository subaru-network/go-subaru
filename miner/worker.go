...
// setSubase sets the etherbase used to initialize the block coinbase field.
func (w *worker) setSubase(addr common.Address) {
	w.mu.Lock()
	defer w.mu.Unlock()
	w.coinbase = addr
}
...
func (w *worker) commit(env *environment, interval func(), update bool, start time.Time) error {
	if w.isRunning() {
		...
		// https://github.com/subaru-network/go-subaru/issues/24299
		env := env.copy()
...
func totalFees(block *types.Block, receipts []*types.Receipt) *big.Float {
	...
	return new(big.Float).Quo(new(big.Float).SetInt(feesWei), new(big.Float).SetInt(big.NewInt(params.Su)))
}
