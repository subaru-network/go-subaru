...
type Config struct {
	Subase  common.Address `toml:",omitempty"` // Public address for block mining rewards (default = first account)
	...
	for {
		...
		case ev := <-dlEventCh:
			...
			case downloader.FailedEvent:
				canStart = true
				if shouldStart {
					miner.SetSubase(miner.coinbase)
					miner.worker.start()
				}
...
func (miner *Miner) SetSubase(addr common.Address) {
	miner.coinbase = addr
	miner.worker.setSubase(addr)
}
